package com.ixs.mvctry.util.dijkstra.model;
...
public class Graph {
	 ...
	 * 
	 */
	public void calculateShortestDistances(int sourse) {
		
		// Set node 0 as the source
		this.nodes[sourse].setDistanceFromSource(sourse);
		int nextNode = sourse;
	 ...
	 * 
	 */
	public String toString(int sourse) {

		String output = "Number of nodes = " + this.noOfNodes;
		output += "\nNumber of edges = " + this.noOfEdges;

		for (int i = 0; i < this.nodes.length; i++) {
			output += ("\nThe shortest distance from node "+sourse+" to node " + i + " is " + nodes[i].getDistanceFromSource());
//保存到数据库
